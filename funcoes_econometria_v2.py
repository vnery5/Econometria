...
#Pacotes de fórmula patsy no SM
...
def get_data(name = ""):
    ...
    if name == "":
        try:
            ...
            print(f"{file}.dta foi lido com sucesso!")
            return df
        except:
        ...
        try:
            ...
            df = pd.read_stata(file)
            print(f"{name}.dta foi lido com sucesso!")
            return df
        except:
            try:
...
def OLS_reg(formula, data, cov='normal'):
    """
...
def poisson_reg(formula, data, model = probit, cov ='normal'):
    """
